...
interface Props {
    medicamento: {
        ...
        id_principio_ativo: number | null;
        id_classificacao: number | null;
        id_sintomatologia: number | null;
        id_alt_lab: number | null;
        id_interacao: number | null;
        id_acao_med: number | null;
        id_acao_nut: number | null;
        id_acao_enf: number | null;
...
const form = useForm({
    nome_comercial: props.medicamento.nome_comercial,
    id_principio_ativo: props.medicamento.id_principio_ativo || '',
    id_classificacao: props.medicamento.id_classificacao || '',
    id_sintomatologia: props.medicamento.id_sintomatologia || '',
    id_alt_lab: props.medicamento.id_alt_lab || '',
    id_interacao: props.medicamento.id_interacao || '',
    id_acao_med: props.medicamento.id_acao_med || '',
...
function submit() {
    form.put(medicamentosUpdate(props.medicamento.id_medicamento).url);
}
</script>

<template>
    <Head :title="'Editar ' + medicamento.nome_comercial" />
    ...
    <div class="flex h-full flex-1 flex-col gap-6 p-4 lg:p-8">
        <div class="rounded-xl border border-sidebar-border/70 bg-white p-6 shadow-sm dark:border-sidebar-border dark:bg-zinc-900">
            <h1 class="mb-6 text-2xl font-bold text-zinc-900 dark:text-white">Editar Medicamento</h1>
